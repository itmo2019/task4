...
function isLetterReady(letter, expectedCount) {
...
function generateWords(isTheme) {
    let result = [];
    let count = 0;
    if (isTheme) {
    ...
    for (let i = 0; i < count; i++) {
        result.push(letterWords[getRandomArbitrary(0, letterWords.length - 1)]);
    }
    return result;
}

function genSentence(isTheme) {
    let words = generateWords(isTheme);
    addCommas(words);
    const signs = [".", "!", "?", "..."];
...
function genLetterText(isTheme) {
    ...
    while (!isLetterReady(letter, count)) {
