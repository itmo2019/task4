...
function generateLetter() {
    let firstletter = document.getElementsByClassName("letter_hidden")[0];
...
function addletter() {
    ...
    let timeout = randomInt(Math.max(10, Math.min(currentTime - beforeTime, 300000)), 600000);
    beforeTime = previousTime;
    previousTime = currentTime + timeout;
    let newLetter = generateLetter();
    let letters = document.getElementsByClassName("letters")[0];
    if (letters.length - hiddenLetters.length === 31) {
        hiddenLetters.unshift(letters[29]);
        letters[29].setAttribute("data-state", "hidden");
    }
...
function deleteLetters() {
    ...
    while (i < letter.length) {
        ...
        if (check.length > 0) {
            if (check[0].checked) {
                let deleteLetter = letter[i];
                letter[i].setAttribute("data-state", "deleted");
                letter[i].addEventListener("animationend", () => {
    ...
    if (hiddenLetters.length > 0) {
        for (let i = 0; i < Math.min(hiddenLetters.length, 31 - (letters.length - hiddenLetters.length)); i++) {
            hiddenLetters[0].setAttribute("data-state", "showed");
            hiddenLetters[0].addEventListener('animationend', () => {
                hiddenLetters[0].removeAttribute("data-state")
            });
            hiddenLetters.shift();
        }
    }
...
function showLetters() {
    document.getElementsByClassName("letters")[0].innerHTML = content;
}

let content = '';

function innerMail(letter) {
    content = document.getElementsByClassName("letters")[0].innerHTML;
    document.getElementsByClassName("letters")[0].innerHTML = letter.children[6].innerHTML;
}

function f() {
    let letters = document.getElementsByClassName("letter");
    for (let i = 0; i < letters.length; i++) {
        letters[i].children[6].innerHTML = generateText();
        letters[i].addEventListener('click', () => {innerMail(letters[i])});
    }
    content = document.getElementsByClassName("letters")[0].innerHTML;
    ...
    setTimeout(addletter, randomInt(10, 600000));
}